...
SELECT        dbo.TasksItems.id AS Id,dbo.TasksItems.UserId, dbo.TasksItems.Shared, dbo.TasksItems.Title, dbo.TasksItems.Notes, dbo.TasksItems.Completed, dbo.TasksItems.DueDate, dbo.TasksItems.Priority, dbo.TasksItems.Type, dbo.TasksItems.Tags, 
                         dbo.TasksItems.[Order],dbo.TasksItems.Warning, dbo.PrivateViewIdentityServerAdminUserList.FullName AS Owner, CASE WHEN Completed = 1 THEN 'V' ELSE 'R' END AS CompletionStatus, CASE WHEN DATEDIFF(day, GETDATE(), DueDate) 
                         < 0 THEN 'R' WHEN DATEDIFF(day, GETDATE(), DueDate) < Warning THEN 'G' ELSE 'V' END AS DeadlineStatus, dbo.TasksItems.Disabled
