...
$targetContexts = @{ 
    IdentityDbConnection= "Migrations\Identity"
}
...
Write-Host "Start remove migration"
foreach ($provider in $dpProviders.Keys) {
    ...
    if ($migrationProviderName -eq 'All' -or $migrationProviderName -eq $provider) {
        ...
        Write-Host "Remove migration for db provider:" $provider ", for project path - " $projectPath
