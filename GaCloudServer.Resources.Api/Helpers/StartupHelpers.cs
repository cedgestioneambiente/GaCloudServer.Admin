...
using GaCloudServer.Resources.Api.Configuration.ApplicationParts;
using GaCloudServer.Resources.Api.Configuration.AuditLogging;
using GaCloudServer.Resources.Api.Configuration.Constants;
using GaCloudServer.Resources.Api.Helpers.Localization;
...
namespace GaCloudServer.Resources.Api.Helpers
{
    public static class StartupHelpers
    {
        ...
        public static IServiceCollection AddAdminApiCors(this IServiceCollection services, ResourcesApiConfiguration resourcesApiConfiguration)
        {
            services.AddCors(options =>
            {
                options.AddDefaultPolicy(
                    builder =>
                    {
                        if (resourcesApiConfiguration.CorsAllowAnyOrigin)
                        {
                            builder.AllowAnyOrigin();
                        }
                        else
                        {
                            builder.WithOrigins(resourcesApiConfiguration.CorsAllowOrigins);
                        }
        ...
        /// <summary>
        /// Register DbContexts for Resources
        /// Configure the connection strings in AppSettings.json
        /// </summary>
        /// <typeparam name="TResourcesDbContext"></typeparam>
        /// <param name="services"></param>
        /// <param name="configuration"></param>
        public static void AddResourcesDbContexts<TResourcesDbContext>(this IServiceCollection services, IConfiguration configuration)
            where TResourcesDbContext : DbContext
        {
            var databaseProvider = configuration.GetSection(nameof(DatabaseProviderConfiguration)).Get<DatabaseProviderConfiguration>();
            var databaseMigrations = configuration.GetSection(nameof(ExtendedDatabaseMigrationsConfiguration)).Get<ExtendedDatabaseMigrationsConfiguration>() ?? new ExtendedDatabaseMigrationsConfiguration();
            var connectionStrings = configuration.GetSection("ConnectionStrings").Get<ExtendedConnectionStringConfiguration>();
        ...
        public static void AddApiAuthentication<TIdentityDbContext, TUser, TRole>(this IServiceCollection services,
            ...
            where TUser : class
        {
            var adminApiConfiguration = configuration.GetSection(nameof(ResourcesApiConfiguration)).Get<ResourcesApiConfiguration>();
        ...
        {
            var resourcesApiConfiguration = services.BuildServiceProvider().GetService<ResourcesApiConfiguration>();
                ...
                options.AddPolicy(AuthorizationConsts.AdministrationPolicy,
                    policy =>
                        policy.RequireAssertion(context => context.User.HasClaim(c =>
                            ...
                            ) && context.User.HasClaim(c => c.Type == JwtClaimTypes.Scope && c.Value == resourcesApiConfiguration.OidcApiName)
                        ));
            });
        }

        public static void AddIdSHealthChecks<TConfigurationDbContext, TPersistedGrantDbContext, TIdentityDbContext, TLogDbContext, TAuditLoggingDbContext, TDataProtectionDbContext, TResourcesDbContext>(this IServiceCollection services, IConfiguration configuration, ResourcesApiConfiguration resourcesApiConfiguration)
            where TConfigurationDbContext : DbContext, IAdminConfigurationDbContext
            where TPersistedGrantDbContext : DbContext, IAdminPersistedGrantDbContext
            where TIdentityDbContext : DbContext
            where TLogDbContext : DbContext, IAdminLogDbContext
            ...
            where TResourcesDbContext: DbContext,IResourcesDbContext
        {
            ...
            var resourcesDbConnectionString = configuration.GetConnectionString(ConfigurationConsts.ResourcesDbConnectionStringKey);

            var identityServerUri = resourcesApiConfiguration.IdentityServerBaseUrl;
            var healthChecksBuilder = services.AddHealthChecks()
                ...
                .AddDbContextCheck<TResourcesDbContext>("ResourcesDbContext")
            ...
            using (var scope = scopeFactory.CreateScope())
            {
                ...
                var ResourcesTableName = DbContextHelpers.GetEntityTable<TResourcesDbContext>(scope.ServiceProvider);
